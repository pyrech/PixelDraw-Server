...
var pd_word = "";
function HTML5_get_current_word(){pd_word =""; return pd_word;} //String

var pd_current_room = {};
function HTML5_am_I_admin(){if (pd_current_room.admin_id == pd_my_id) {return 1;} else {return 0;}}//Int
function HTML5_get_admin(){if (pd_current_room.admin_id) {return pd_current_room.admin_id;} else {return "";}}//Int
...
function join_room(data) {
  ...
  //pd_current_players = data.room.players; //Fixme
...
function HTML5_send_message(msg){
  sess.publish(pd_room_id, {player_message:'msg test de remi'},false);
}
...
function HTML5_create_room(){
  sess.call("create_room",{}).then(create_room_cb, log_call_result_error);
...
function HTML5_leave_room(){
  if (pd_room_id != ""){
    ...
    cconsole.log("WARNING: trying to leave room, but not in any room");
  }
}
...
function HTML5_get_room_players(){
  sess.call("get_room_players",{}).then(get_room_players_cb, log_call_result_error);
...
function pd_onEvent(topicUri, data) {
  ...
  switch (data.type) {
    case 0: console.log("game event received is:",0,"GamePlayerEvent:",data); pd_messages.push(data.msg); break;
    case 1: console.log("game event received is:",1,"GameServerEvent:",data); pd_messages.push(data.msg); break;
    case 2: 
    ...
    case 3: 
      ...
      pd_current_colors = data.picture;
    break;
  }
}
